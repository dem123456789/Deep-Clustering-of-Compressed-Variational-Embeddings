...
def runExperiment(seed):
    ...
    model = eval('models.{}.{}(test_dataset.classes_size).to(device)'.format(model_dir,model_name))
...
def test(validation_loader,model,epoch,protocol,iter,model_TAG):
    ...
    with torch.no_grad():
        ...
        for i, input in enumerate(validation_loader):
            ...
            protocol = update_protocol(input,iter,protocol)  
...
def update_protocol(input,iter,protocol):
...
def print_result(epoch,iter,test_meter_panel):
    print('Test Epoch: {}({}){}'.format(epoch,iter,test_meter_panel.summary(['loss','bpp','psnr','acc'])))
    return
