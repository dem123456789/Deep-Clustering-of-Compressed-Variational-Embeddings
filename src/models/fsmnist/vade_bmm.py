...
class vade_bmm(nn.Module):
	def __init__(self,classes_size):
		...
		self.param = nn.ParameterDict({
			...
			# 'mean': nn.Parameter(torch.ones(config.PARAM['code_size']*config.PARAM['num_level'], self.classes_size)/config.PARAM['num_level']),
	...
	def classifier(self, input, protocol):        
		...
		if torch.sum(torch.isnan(q_c_z)):
			print(p_y_c)
			print(torch.sum(torch.isnan(z[:,:,1,:]*torch.log(p_y_c))))
			print(torch.sum(torch.isnan(z[:,:,0,:]*torch.log(1-p_y_c))))
			print(torch.sum(torch.isnan(torch.log(self.param['pi']))))
		return q_c_z
	...
	def classification_loss_fn(self, input, output, protocol):
		...
		z = z[:,:,1,:] #100x32x1
		p_y_c = custom_replace(torch.sigmoid(self.param['mean']),noise=1e-7)
		if(protocol['tuning_param']['classification'] > 0): 
			q_c_z = output['classification']['classifier'] #NxH
			q_y = output['compression']['param']['qy'].view(input['img'].size(0),config.PARAM['code_size'],config.PARAM['num_level'],1) #p(z=1|x) 128x32x1
