...
code_size = 128
z_dim = 10
categorical_dim = 2  # one-of-K vector bernoulli
temp = 1.0
temp_min = 0.5
ANNEAL_RATE = 0.00003
...
class Joint_cnn_bmm(nn.Module):
	def __init__(self,classes_size):
		super(Joint_cnn_bmm, self).__init__()
		...
		self.fc_q = nn.Linear(2048, z_dim*categorical_dim)
		self.fc0= nn.Linear(z_dim*categorical_dim, 2048)
	...
	def forward(self, input, protocol):
		...
		q = self.fc_q(encoded)
		qy = q.view(q.size(0),z_dim,categorical_dim)
		code = gumbel_softmax(logits=q_y, temperature=temp)
		param = {'qy':F.softmax(q_y, dim=-1).reshape(*q.size())}
