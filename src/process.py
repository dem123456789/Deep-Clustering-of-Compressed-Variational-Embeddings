...
def main():
    control_names = [['0'],[config.PARAM['data_name']['train']],['vade','cvade','vadebmm','cvadebmm'],['28']]
...
def extract_result(model_TAGs):
    head = './output/model/milestones_0/01'
    tail = 'best'
...
def gather_result(model_TAGs):
    ...
    for i in range(len(model_TAGs)):
        ...
        if(os.path.exists(result_path)):
            ...
            gathered_result[model_TAGs[i]] = {'x':0,'loss':result['test_meter_panel'].panel['loss'].history_avg[-1],
            'cluster_acc':result['test_meter_panel'].panel['cluster_acc'].val}
        else:
...
def process_result(control_names):
    ...
    result = load(result_path)
    evaluation_names = ['x','loss','cluster_acc']
    processed_result = {}
...
def show_result():
    ...
    fig = plt.figure()
    for model_TAG in result['indices']:
        x = result['mean']['x'][tuple(result['indices'][model_TAG][1:])].cpu().numpy()
